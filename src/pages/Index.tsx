...
const Index = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [sectionsRes, categoriesRes, combosRes, productsRes] = await Promise.all([
          productsAPI.getSections().catch(() => ({ results: [] })),
        ...
        setCombos(combosRes.results || combosRes || []);
        setAllProducts(productsRes.results || productsRes || []);
      } catch (error) {
  ...
  // Get section products or fallback
  const getSectionProducts = (sectionType: string) => {
    ...
    return allProducts.slice(0, 4).map((p, i) => formatProduct(p, i));
  };
  ...
  const formattedCombos = combos.slice(0, 4).map((c, i) => formatCombo(c, i));
          ...
          <section className="py-10">
            <div className="container mx-auto px-2 sm:px-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 gap-2 sm:gap-8 max-w-4xl mx-auto">
                {formattedCombos.map(product => <ProductCard key={product.id} {...product} />)}
              </div>
            </div>
          </section>
        )}
