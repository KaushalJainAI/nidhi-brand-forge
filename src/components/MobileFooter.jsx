...
import { Home, ShoppingBag, Gift, User, Heart } from "lucide-react";
...
const MobileFooterNav = () => {
  ...
  // Example: cart badge on 'Offers' tab or 'Profile', you can relocate as needed.
  // Replace below if you want badge on Cart or another tab.

  return (
    <nav className="fixed bottom-0 z-50 w-full bg-card border-t border-border flex justify-around md:hidden h-16">
      {footerItems.map((item, i) => {
        const Icon = item.icon;
        // Cart badge example on Profile
        return (
          <Link
            key={i}
            to={item.path}
          ...
          >
            <span className="relative">
              ...
              {item.label === "Profile" && cart.length > 0 && (
